...
def f_repos(*args,**kwargs):
    """
        self defined module of repositioning
    """
    import random
    repos = DotMap()
    repos.marks = []
    threshold = 600     # justified as non-adjacent hexagons if distance exceeds this threshold
    driver = kwargs.get('veh',None)
    sim = driver.sim 
    pos_hash_ind = str(sim.inData.ref[sim.inData.ref['node']== driver.veh.pos]['hash'].values[0])
    neighbors = list(h3.hex_ring(pos_hash_ind,1))
    neighbors.append(pos_hash_ind)
    neighbors = [x for x in neighbors if str(x) in sim.inData.centers['hash'].values]
    while(1):
        repos_pos_ind = random.choice(neighbors)
        repos_pos = sim.inData.centers[sim.inData.centers['hash']==repos_pos_ind]['node'].values[0]
        repos_time = driver.sim.skims.ride[driver.veh.pos][repos_pos]
        if repos_time > threshold:
            neighbors.remove(repos_pos_ind)
            continue
        else:
            repos.pos = repos_pos
            repos.time = repos_time
            repos.flag = True
            repos.marks.append(repos_pos_ind)
            driver.veh.pos = repos.pos
            break

    return repos

def find_neighbors(pos_hash_ind,learn_driver,driver):
    """
        self defined module of finding neighbors and their PGAM, examine whether they are neighbors
    """
    sim = driver.sim 
    threshold = 600
    ...
    for key in neighbors:
        if key in learn_driver:
            neighbor_keys.append((key, learn_driver[key][2]))
        else:
            neighbor_keys.append((key,0.2))    
    df = pd.DataFrame(neighbor_keys,columns=['hash','PGAM'])
    pos = sim.inData.centers[sim.inData.centers['hash']==pos_hash_ind]['node'].values[0]
    for hash in df['hash']:
        pos1 = sim.inData.centers[sim.inData.centers['hash']==hash]['node'].values[0]
        repos_time = driver.sim.skims.ride[pos][pos1]
        if repos_time > threshold:
            df = df.loc[df['hash']!=hash]
 
    return df

def f_find_max(pos_ind,df,driver,flag):
    """
       find the maximum 
    """
    sim = driver.sim 
    repos = DotMap()
    if flag == True:
        max = df[df['PGAM'] == df['PGAM'].max()].iloc[0]
        repos_score = max['PGAM']
    else:
        max = df[df['Scores'] == df['Scores'].max()].iloc[0]
        repos_score = max['Scores']
    repos_pos_ind = max['hash']
    repos_pos = sim.inData.centers[sim.inData.centers['hash']==repos_pos_ind]['node'].values[0]
    pos = sim.inData.centers[sim.inData.centers['hash']==pos_ind]['node'].values[0]
    repos_time = driver.sim.skims.ride[pos][repos_pos]

    repos.pos = repos_pos
    repos.time = repos_time
    repos.flag = True
    repos.score = repos_score  
    repos.max = max
    repos.ind = repos_pos_ind  
    #mark_on_map(sim.inData,marks,'red')
    return repos

def f_repos_score(*args,**kwargs):
    """
        self defined model of repositioning according to score
    """
    # justified as non-adjacent hexagons if distance exceeds this threshold
    driver = kwargs.get('veh',None)
    learn = kwargs.get('learn',None)
    pos = kwargs.get('pos',None)
    learn_driver = learn[driver.veh.name]
    sim = driver.sim 
    print(sim.inData.ref[sim.inData.ref['node']== pos])
    pos_hash_ind = str(sim.inData.ref[sim.inData.ref['node']== driver.veh.pos]['hash'].values[0])
    df = find_neighbors(pos_hash_ind,learn_driver,driver)
    max_scores = []
    for index,row in df.iterrows():
        print(row)
        df1 = find_neighbors(row['hash'],learn_driver,driver)
        max_scores1 = []
        for index,row1 in df1.iterrows():
            df2 = find_neighbors(row1['hash'],learn_driver,driver)
            max1 = f_find_max(row1['hash'],df2,driver,True)
            max_row1 = max1['max']
            max_score1 = max_row1['PGAM'] * (1-row1['PGAM']) * 0.7 + row1['PGAM']
            max_scores1.append(max_score1)
        df1['Scores'] = pd.DataFrame(max_scores1)
        max = f_find_max(row['hash'],df1,driver,False)
        max_row = max['max']
        max_score = max_row['Scores'] * (1-row['PGAM']) * 0.7 + row['PGAM']
        max_scores.append(max_score)
    df['Scores'] = pd.DataFrame(max_scores)
    #score = repos.score
    #repos = f_find_max (df,driver)
    repos = f_find_max(pos_hash_ind,df,driver,False) 
    driver.veh.pos = repos.pos
    return repos
