...
class VehicleAgent(object):
    ...
    def __init__(self, simData, inData, veh_id, run_id):
        # ids
        ...
        self.f_repos_score = self.sim.functions.f_repos_score   # reposition based on score
        self.mark_on_map = self.sim.functions.mark_on_map
        # events
        ...
        # plot learned cells on map
        self.mark_learned_cells()
    ...
    def self_learning(self,driverID,locationID,time,activation_flag):
        if locationID != 0:
            location = str(self.ref[self.ref['node']==locationID]['hash'].values[0])
        else:
            location = locationID

        if driverID not in self.learn:
            self.learn[driverID] = {}
        if location not in self.learn[driverID]:
            self.learn[driverID][location] = [0,0,0.2]

        a = self.learn[driverID][location]
        total_time = a[0]+time
        total_success = a[1]+activation_flag
        if total_time != 0:
            PGAM = float(total_success)/float(total_time)*60
        else:
            PGAM = 1
        self.learn[driverID][location] = [total_time,total_success,PGAM]
        return self.learn
        ...
        while True:
            ...
            while True:
                if self.day <= 5:
                    ...
                    repos = self.f_repos_score(learn=self.learn,veh=self)
                self.marks.extend(repos.marks)
                ...
                time1 = self.sim.env.now
                yield self.requested | self.sim.timeout(min(599,self.till_end())) # 5 minutes
                time2 = self.sim.env.now
                self.update(event=driverEvent.REPOSITIONED, pos=repos.pos)
                if self.till_end() == 0:
                    self.self_learning(self.id,repos.pos,time2-time1,0)
                    break
                    #self.platform.vehQ.pop(self.platform.vehQ.index(self.id))
                if self.requested.triggered:
                    self.self_learning(self.id,0,time2-time1,1)
                    break
                else:
                    self.self_learning(self.id,repos.pos,600,0)   # learning
                    first_time = False
            ...
            if self.schedule is None:
                ...
                for i in range(1, self.schedule.shape[0]):  # loop over the schedule
                    ...
                    if stage.od == 'o':  # pickup
                        ...
                        if 0 in self.learn[self.id]:   #update the learning process
                            hash = str(self.ref[self.ref['node']==self.veh.pos]['hash'].values[0])
                            self.learn[self.id][hash] = self.learn[self.id][0]
                            self.learn[self.id].pop(0)
                            self.marks.append(hash)
                            self.pickups.append(hash)
                        print(str(self.id) +' matched with '+ str(stage.req_id))
            ...
            if self.exit_flag:
                ...
                self.sim.logger.info(msg)
                if self.id in self.learned:
                    self.learned[self.id].extend(self.marks)
                else:
                    self.learned[self.id] = self.marks
                if self.id in self.requests:
                    self.requests[self.id].extend(self.pickups)
                else:
                    self.requests[self.id] = self.pickups
                #base_map2 = self.mark_on_map(self.map,self.pickups,'green')
                base_map = self.mark_on_map(self.map,self.marks,'red')
                base_map.save('trajectory/map_veh'+str(self.id)+'_day'+str(self.day)+'.html')
                #base_map2.save('trajectory/pickup_veh'+str(self.id)+'_day'+str(self.day)+'.html')
                break       
        
        import pandas as pd
        learn_df = pd.DataFrame.from_dict(self.learn)
        #print(learn_df)

    def mark_learned_cells(self):
        
        #print(self.learned)
        #for key in self.learned:
        #    base_map = self.mark_on_map(self.map,self.learned[key],'green')
        #    base_map.save('trajectory/map_'+str(key)+'.html')
        return self.marks,self.pickups
