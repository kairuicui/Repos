...
from MaaSSim.decisions import f_repos, f_match, dummy_False,f_repos_score
from MaaSSim.platform import PlatformAgent
from MaaSSim.performance import kpi_pax, kpi_veh
from MaaSSim.utils import initialize_df,mark_on_map
import sys
import logging
...
DEFAULTS = dict(f_match=f_match,
                ...
                f_repos_score=f_repos_score,
                mark_on_map = mark_on_map,
...
class Simulator:
    ...
    def generate(self,run_id):
        ...
        for veh_id in self.vehicles.index:
            self.vehs[veh_id] = VehicleAgent(self, self.inData, veh_id, run_id)
            #relocates,requests = self.vehs[veh_id].mark_learned_cells()
            #base_map = mark_on_map(self.inData.map,relocates,'red')
            #base_map.save('trajectory/map_veh'+str(veh_id)+'.html')
