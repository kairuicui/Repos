...
KPIs_descriptions = ['total travel time of vehicles (with travellers only)',
                     ...
                     'total fares for shared vehicles',
                     'total fares for non-shared vehicles',
                     'total fares for two service',
                     'total fares paid by travellers sharing',
                     'as above yet in non-shared scenarion ',
                     'relative revenue reduction',
                     'number of trips',
                     'number of single rides in the solution',
                     'number of public transport rides in the solution',
...
def generate_demand(_inData, _params=None, avg_speed=False):
    ...
    requests.origin = list(
        distances.sample(_params.nP*3, weights='p_origin', replace=True).index)  # sample origin nodes from a distribution
    requests.destination = list(distances.sample(_params.nP*3, weights='p_destination',
    ...
    requests = requests.loc[0:_params.nP,:]
    print('minimum value:')
    print(requests['dist'].min())
...
def synthetic_demand_poly(_inData, _params=None):
    ...
    Centers['node'] = Centers.apply(lambda center: nearest_nodes(_inData.G, center.x, center.y), axis=1)
...
def mode_choices(_inData, sp):
    ...
    PT = rides[rides.max_prob_PT >= 0.4]
