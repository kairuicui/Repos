...
def single_rides(_inData, params):
    ...
    def f_delta():
        ...
        return (1 / params.WtS - 1) * req.ttrav * params.beta_cost + \
    ...
    def utility_PT():
        ...
        #params.PT_speed = 1/2
        #params.ascpt = 1
        if params.get("PT_price") is None or params.get("PT_beta") is None:
            return 999999
        else:
            return (params.PT_price * req.dist / 1000 * params.beta_cost+
                  req.VoT * params.PT_beta * (req.ttrav / params.PT_speed) * params.beta_cost+
                  params.walk_beta * params.VoT * params.walk_time * params.beta_cost+99999)
    
    def utility_bike():
        bike_speed = 10
        # utility of trip with bike
        if params.get("bike_beta") is None:
            return 999999
        else:
            return params.VoT * params.bike_beta*req.dist/(1000 * bike_speed) #TBD
    ...
    if 'VoT' not in req.columns:
        if params.get('VoT_std', False):
            ...
            req['VoT'] = exponential(params.VoT, params.nP)
        else:
    ...
    req['u'] = params.price * req.dist / 1000 * params.beta_cost + req.VoT * req.ttrav * params.beta_cost
    ...
    df.loc[df['u_PT']<=df['u'],'kind'] = SbltType.PT.value
    df['indexes'] = df.index
...
def pairs(_inData, params, process=True, check=True, plot=False):
    ...
    def utility_pt_i():
        return (params.PT_price * r.dist_i / 1000 * params.beta_cost+ \
            r.VoT_i * (r.ttrav_i / params.PT_speed) * params.beta_cost + \
            params.walk_beta * r.VoT_i * params.walk_time * params.beta_cost+99999) #+ 999999

    def utility_pt_j():
        return (params.PT_price * r.dist_j / 1000 * params.beta_cost+ \
            r.VoT_j * (r.ttrav_j / params.PT_speed) * params.beta_cost +\
            params.walk_beta * r.VoT_j * params.walk_time * params.beta_cost+99999) #+ 999999
    ...
    def utility_ns_i():
        ...
        #print('ns:'+str(params.price * r.dist_i / 1000 + r.VoT_i * r.ttrav_i))
        return params.price * r.dist_i / 1000 * params.beta_cost + r.VoT_i * r.ttrav_i * params.beta_cost

    def utility_ns_j():
        # utility of non shared trip j
        return params.price * r.dist_j / 1000 * params.beta_cost + r.VoT_j * r.ttrav_j * params.beta_cost

    def utility_sh_i():
        # utility of shared trip #
        return (params.price * (1 - params.shared_discount) * r.dist_i / 1000 * params.beta_cost +\
                r.VoT_i * params.WtS * (r.t_oo + params.pax_delay + r.t_od +\
                      params.delay_value * abs(r.delay_i)) * params.beta_cost)
    
    def utility_sh_j():
        # utility of shared trip j
        return (params.price * (1 - params.shared_discount) * r.dist_j / 1000 * params.beta_cost +\
                r.VoT_j * params.WtS * (r.t_od + r.t_dd + params.pax_delay +\
                                        params.delay_value * abs(r.delay_j)) * params.beta_cost)
    ...
    def utility_shpt_i():
        sh = params.price * (1 - params.shared_discount) * r.dist_i / 1000 + \
                r.VoT_i * params.WtS * (r.t_oo + params.pax_delay + r.t_od + params.delay_value * abs(r.delay_i))
        pt = params.price * (1-params.PT_discount) * r.dist_i / 1000 + \
            r.VoT_i * params.PT_beta * (r.ttrav_i / params.PT_speed)
        return (sh - pt)
    
    def utility_shpt_j():
        return (params.price * (1 - params.shared_discount) * r.dist_j / 1000 +\
                r.VoT_j * params.WtS * (r.t_od + r.t_dd + params.pax_delay + params.delay_value * abs(r.delay_j))-
                params.price * (1-params.PT_discount) * r.dist_j / 1000 + r.VoT_j * params.PT_beta * (r.ttrav_j / params.PT_speed))
    ...
    def utility_j_LIFO():
        # utility of LIFO trip for i
        return (params.price * r.dist_i / 1000 * params.shared_discount * params.beta_cost
                + r.VoT_j * (r.ttrav_j - params.WtS * (r.t_od + params.delay_value * abs(r.delay_j)))) * params.beta_cost

    def utility_sh_i_LIFO():
        # difference u_sh_i_LIFO - u_ns_i
        return (params.price * (1 - params.shared_discount) * r.dist_i / 1000 * params.beta_cost +
                r.VoT_i * params.WtS * (r.t_oo + r.t_od + r.t_dd + 2 * params.pax_delay + params.delay_value * abs(r.delay_i))) * params.beta_cost

    def utility_sh_j_LIFO():
        # difference u_sh_j_LIFO - u_ns_j
        return (params.price * (1 - params.shared_discount) * r.dist_j / 1000 * params.beta_cost +
                r.VoT_j * params.WtS * (r.t_od + params.delay_value * abs(r.delay_j))) * params.beta_cost
    
    def utility_shpt_i_LIFO():
        return -(params.price * (1-params.PT_discount) * r.dist_i / 1000 + \
            r.VoT_i * params.PT_beta * (r.ttrav_i / params.PT_speed) - \
            (params.price * r.dist_i / 1000 * params.shared_discount
                + r.VoT_i * (r.ttrav_i - params.WtS * (
                        r.t_oo + r.t_od + 2 * params.pax_delay + r.t_dd + params.delay_value * abs(r.delay_i))))    )

    def utility_shpt_j_LIFO():
        return (params.price * r.dist_i / 1000 * params.shared_discount
                + r.VoT_j * (r.ttrav_j - params.WtS * (r.t_od + params.delay_value * abs(r.delay_j)))-
                (params.price * (1-params.PT_discount) * r.dist_j / 1000 + \
            r.VoT_j * params.PT_beta * (r.ttrav_j / params.PT_speed)))
    
    def utility_analysis():
        df1 = pd.concat([utility_ns_i(),utility_sh_i()],axis=1)
        df1 = pd.concat([df1,utility_pt_i()],axis=1)
        #df1 = pd.concat([df1,utility_ns_j()],axis = 1)
        #df1 = pd.concat([df1,utility_sh_j()],axis = 1)
        #df1 = pd.concat([df1,utility_pt_j()],axis = 1)
        df1.columns = ['ns_i','sh_i','pt_i']#,'ns_j','sh_j','pt_j']
        print(df1)
        df1.to_csv('utilities.csv')
    ...
    def check_me_FIFO():
        ...
        try:
            ...
            assert (r.u_i <= utility_pt_i() * 1.01).all()
            assert (r.u_j <= utility_pt_j() * 1.01).all()
            ...
            _inData.logger.warning(r[~(r.u_j <= utility_ns_j() * 1.01)])  
            _inData.logger.warning(r[~(r.u_i <= utility_pt_i() * 1.01)])  # do we have positive sharing utility compared to pt
            _inData.logger.warning(r[~(r.u_j <= utility_pt_j() * 1.01)])
    ...
    print('number of ns and pt users:')
    #print(len(r[(utility_ns_i() <= utility_sh_i()) & (utility_ns_i() <= utility_pt_i())]))
    #print(len(r[(utility_pt_i() <= utility_sh_i()) & (utility_pt_i() <= utility_ns_i())]))
    #print(len(r[(utility_sh_i() <= utility_pt_i()) & (utility_sh_i() <= utility_ns_i())]))
    #r = r[(utility_sh_i() <= utility_pt_i()) & (utility_sh_i() <= utility_ns_i())]
    #print('utility<0')
    if plot:
    ...
    r = r[utility_j() > 0]
    #r = r[(utility_sh_j() <= utility_pt_j()) & (utility_sh_j() <= utility_ns_j())]
    print(len(r))
    if plot:
    ...
    #r = r[(utility_sh_i_LIFO() <= utility_pt_i()) & (utility_sh_i_LIFO() <= utility_ns_i())]
    #r = r[(utility_sh_j_LIFO() <= utility_pt_j()) & (utility_sh_j_LIFO() <= utility_ns_j())]
...
def evaluate_shareability(_inData, params, plot=False):
    ...
    ret['VehHourTrav_ns'] = r1.ttrav.sum()

    ret['PassHourTrav'] = r1.ttrav_sh.sum()
    ret['PassHourTrav_ns'] = r1.ttrav.sum()

    ret['PassUtility'] = r1.u_sh.sum()
    ret['PassUtility_ns'] = r1.u.sum()
    ...
    ret['shared_fares'] = schedule[schedule.kind > 1].PassHourTrav_ns.sum() * params.avg_speed * params.price * (
           1 - params.shared_discount)/1000
    ret['ns_fares'] = schedule[schedule.kind == 1].PassHourTrav_ns.sum() * params.avg_speed * params.price / 1000
    ret['revenue'] = ret['shared_fares'] + ret['ns_fares']
    ...
    ret['avg_sh_dist'] = schedule[schedule.kind > 1].PassHourTrav_ns.mean() * params.avg_speed / 1000
    ret['avg_ns_dist'] = schedule[schedule.kind == 1].PassHourTrav_ns.mean() * params.avg_speed / 1000
    ret['avg_dist'] = 1
    ...
    ret['PT'] = schedule[(schedule.kind == 0)].shape[0]
    ...
    import matplotlib.pyplot as plt
    dt = pd.DataFrame({"shared":schedule[schedule.kind > 1].PassHourTrav_ns * params.avg_speed / 1000,
        "non-shared":schedule[schedule.kind == 1].PassHourTrav_ns * params.avg_speed / 1000})
    dt.boxplot()
    plt.ylabel("travel distances")
    plt.xlabel("services")
    plt.savefig("D:\\project\\dat\\boxplot"+str(params.price)+"p"+str(params.shared_discount)+".png")
    plt.show()
    ...
    r1['start'] = pd.to_datetime(r1.treq, unit='s')
    r1['end'] = pd.to_datetime(r1.treq + r1.ttrav, unit='s')
    fsns = fleet_size(r1)
